...
- [ ] [AI-Review][LOW] `StringSlice` helper in `settings.go` is defined but unused by settings routes — verify consumer usage or remove [backend/domain/config/sysconfig/settings.go:L139]
- [ ] [AI-Review][LOW] `nonSensitiveFieldsMatch` uses `fmt.Sprint` for value comparison, which may be unreliable for nested types [backend/domain/routes/settings.go:L342]