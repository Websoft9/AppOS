...
- AC1: `routes/space.go` reads `maxSizeMB`, `maxPerUser`, `shareMaxMinutes`, `shareDefaultMinutes` from `app_settings` via `settings.GetGroup`; hardcoded `const` block is removed.
...
- [x] Task 1: Seed migration (AC3)
  ...
  - [x] 1.2 In `up()`: call `settings.SetGroup` only if the row does not already exist — check with `app.FindFirstRecordByFilter("app_settings", "module='space' && key='quota'", ...)` first
...
- [x] Task 2: Replace constants in `routes/space.go` (AC1, AC4, AC5)
  - [x] 2.1 Remove the `const` block (`spaceMaxSizeMB`, `spaceMaxPerUser`, `spaceShareMaxMin`, `spaceShareDefaultMin`)
  - [x] 2.2 Add package-level `defaultSpaceQuota` fallback map
  - [x] 2.3 In `handleSpaceQuota`: load quota with `settings.GetGroup`, then use `settings.Int` for each field
  - [x] 2.4 In `handleSpaceShareCreate`: same pattern — load quota then read field
  - [x] 2.5 Keep `spaceReservedFolderNames`, `spaceAllowedUploadFormats`, `spaceEditableFormats` as constants

- [x] Task 3: Replace constants in `hooks/hooks.go` (AC2, AC4)
  - [x] 3.1 Remove `hookSpaceMaxPerUser` constant
  - [x] 3.2 In `validateFileUpload`: load quota before checks
  - [x] 3.3 Keep `hookSpaceAllowedFormats` constant
...
`defaultSpaceQuota` (defined in task 2.2) is the **code-level safety net**. When the DB is unavailable or the seed row is missing, `settings.GetGroup` always returns `(fallback, err)` — never `nil`. The `quota, _ := settings.GetGroup(...)` pattern is therefore safe: quota is always a valid map, and handlers never panic regardless of DB state.

### Key files to touch
```
backend/internal/migrations/1741200001_seed_app_settings.go  ← new
backend/internal/routes/space.go                             ← modify: remove const block, load from DB
backend/internal/hooks/hooks.go                              ← modify: remove const block, load from DB
```

Constants **not** migrated this story (not yet configurable): `spaceReservedFolderNames`, `spaceAllowedUploadFormats`, `spaceEditableFormats`, `hookSpaceAllowedFormats`.

### References
- Settings helper: `backend/internal/settings/settings.go` (created in Story 13.1)
- Hardcoded constants to remove: [backend/internal/routes/space.go](backend/internal/routes/space.go#L18-L27)
...
- `hookDefaultSpaceQuota` (package-level var) used in hooks.go (mirrors `defaultSpaceQuota` in space.go)
- `go build ./...` passes with 0 errors

### File List

- `backend/internal/migrations/1741200001_seed_app_settings.go` (new)
- `backend/internal/routes/space.go` (modified)
