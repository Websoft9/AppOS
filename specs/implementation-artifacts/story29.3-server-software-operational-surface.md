...
- keep the operational UI inside Server Detail as a `Components` tab
- split the tab into `Prerequisites` and `Addons`
- keep `docker` as the only prerequisite component in the first rollout
- render the remaining catalog-backed server components under `Addons`
...
- UI naming and section IA are aligned with Story 20.7

Presentation:

- `Prerequisites` appears first and answers whether the server satisfies the platform baseline
- `Addons` appears second and preserves the compact operational inventory view for all other managed host components
...
- the prerequisite group should prefer a checklist-first visual with compact fact rows over explanation-heavy cards
...
- render Docker Engine version and Docker Compose version or missing state as compact facts
- render platform baseline state as a short checklist rather than paragraph-heavy explanation

Show for each prerequisite:

- label
- compact version facts when relevant
- readiness status
- checklist rows for key baseline checks
- one short blocking summary when degraded
...
- long explanatory prose as the default first-read content
...
- server detail already hosts a server-software detail tab, with naming now converging on `Components`
...
- [ ] Task 2: Complete server detail operational UX
	...
	- [ ] 2.5 keep the existing operational table as `Addons` for the remaining components
	- [ ] 2.6 present prerequisite readiness as a checklist-first block with compact Docker Engine and Docker Compose facts
...
- the Server Detail `Components` tab is split into `Prerequisites` and `Addons`
...
- the prerequisite section uses a checklist-first visual with compact Docker Engine and Docker Compose fact rows
