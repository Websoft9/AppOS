...
1. Initialize all services in all-in-one container with default configurations:
   - ✓ Redis (persistent mode, AOF enabled)
   - ✓ Convex (self-hosted, data directory)
   - ✓ Backend (SQLite schema, default admin user)
   - ✓ Nginx (internal routing configuration)

2. Connection testing suite passes:
   - ✓ Test backend → redis connectivity (Asynq queue)
   - ✓ Test backend → convex connectivity (auth and database)
   - ✓ Test backend → docker socket accessibility
...
- [ ] Task 1: Container-Level Service Initialization (AC: 1)
  ...
  - [ ] Init Backend: Create SQLite DB schema if not exists, ensure config directory
...
- [ ] Task 2: Connection Testing Suite (AC: 2)
  ...
  - [ ] Implement curl-based connectivity tests (cockpit:9090, portainer:9000)
...
│     ├─ Init Redis (create data directory)
│     ├─ Init Convex (create data directory, default config)
│     ├─ Init Backend (SQLite schema, default admin user)
│     └─ Init Nginx (copy config)
...
- Config files managed at container level, not in backend source code
- Portainer init delayed (background, after service starts)
...
**All-in-One Container** (websoft9):
- Single container running: Redis, Convex, Backend, Nginx
- All services initialized within same container
- Services communicate via localhost
- Persistent data in `/websoft9/data` volume
