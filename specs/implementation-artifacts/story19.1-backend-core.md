...
- [x] AC6: `GET /api/secrets/templates` returns all templates from `backend/domain/secrets/templates.json`.
...
- [x] Task 1: Startup guard for `APPOS_SECRET_KEY`
  - [x] 1.1 Load and validate key in `backend/domain/secrets/config.go` at app init
...
- [x] Task 3: PB collection hooks in `backend/domain/secrets/hooks.go`
...
- [x] Task 4: Template loader in `backend/domain/secrets/templates.go`
  - [x] 4.1 Load `templates.json` at startup, fail fast if malformed
  - [x] 4.2 Expose `GET /api/secrets/templates` route
- [x] Task 5: Custom routes in `backend/domain/routes/secrets.go`
  ...
  - [x] 5.4 Register secrets routes via centralized `routes.Register(se)` flow (`backend/domain/routes/routes.go`)
...
- Follows custom route ownership baseline: all `/api/secrets/*` routes live under `backend/domain/routes/`.
...
- `backend/infra/migrations/XXXXXXXXXX_create_secrets.go` — new
- `backend/domain/secrets/config.go` — new
- `backend/domain/secrets/crypto.go` — new (AES-256-GCM encrypt/decrypt; shared with Story 19.3)
- `backend/domain/secrets/hooks.go` — new
- `backend/domain/secrets/templates.go` — new
- `backend/domain/secrets/templates.json` — new
- `backend/domain/routes/secrets.go` — new
- `backend/cmd/appos/main.go` — updated (secrets bootstrap)
- `backend/platform/hooks/hooks.go` — updated (register secrets hooks)
- `backend/domain/routes/routes.go` — updated (register secrets routes)
- `backend/domain/routes/resources.go` — updated (legacy compatibility defaults)
- `backend/domain/routes/secrets_test.go` — new
- `backend/infra/migrations/1762300000_upgrade_secrets_for_epic19.go` — new
