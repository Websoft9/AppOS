...
5. Backend exposes canonical tunnel APIs under `/api/tunnel/*`; any implemented `/api/ext/tunnel/*` routes are migrated and not retained as the long-term contract.
...
- [ ] Migrate tunnel API prefix to canonical domain routes (AC: 5)
  - [ ] Move backend route registration from `/api/ext/tunnel/*` to `/api/tunnel/*`
  - [ ] Update frontend callers to `/api/tunnel/*`
  - [ ] Remove or deprecate legacy `/api/ext/tunnel/*` usage in the implementation
