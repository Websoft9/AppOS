...
- **`audit_logs` uses native PocketBase collection API** (`GET /api/collections/audit_logs/records`); no custom backend endpoint exists or is needed — access control is enforced entirely by PB collection rules (`user_id = @request.auth.id || @request.auth.collectionName = '_superusers'`)
- `GET /api/logs` is a built-in PB superuser-only endpoint (used by the Logs page, not the Audit page)
