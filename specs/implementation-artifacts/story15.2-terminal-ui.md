...
As a superuser, I can use a unified Connect workspace for server terminal commands and basic file operations, so that I can complete routine operations without context switching.
...
- 选择当前已连接的服务器时弹出重连确认对话框（`reconnectNonce` 强制重新挂载 TerminalPanel）
...
- [x] Grid/Tile 视图切换
- [x] 文件列表本地搜索过滤
- [x] Terminal supports multiple active server connections via vertical tab list with collapse/expand
- [x] Terminal header shows location navigation: `Connect > Servers`
- [x] Disconnect flow shows fixed 2-second safe-exit feedback ("Safely disconnecting") before close (non-skippable)
- [x] Connect flow shows minimum 2-second "Establishing secure connection..." feedback before the terminal tab opens, even when the check completes faster (symmetric with disconnect experience)
- [x] In split view, Terminal shell area supports horizontal scrollbar and current shell viewport remains bottom-aligned for active tab
...
- [x] Connect settings are managed by Epic 13 and include idle timeout and max connections (default: `0` unlimited)
...
dashboard/src/pages/connect/ConnectServerPage.tsx               # Multi-connection vertical tabs, safe-exit UX, split presets
dashboard/src/components/connect/TerminalPanel.tsx              # Horizontal scroll + active-tab bottom alignment
dashboard/src/routes/_app/_auth/_superuser/settings.tsx         # Connect Terminal settings UI entry (Epic 13 ownership)
```
...
- `normalizePath` 重写为栈式 `..` 解析，修复路径穿越安全漏洞（原实现仅做 `replace('..','')`）
- 新增 `initialPath`/`lockedRootPath` props，支持从 Volume→Files 跨面板定位
- 新增 `isPathWithinRoot`/`clampPathToRoot` 工具函数，确保导航不越出锁定根路径
- Files 按钮点击时重置 path/lockedRoot/nonce，修复 Volume→Files 后锁定根"粘滞"问题
...
- Implemented multi-connection active sessions with vertical, collapsible tab rail in Connect page.
- Added fixed 2-second non-skippable safe-exit experience before disconnect/tab close.
- Added minimum 2-second "Establishing secure connection..." feedback on connect (connectivity check and 2s delay run concurrently via `Promise.all`).
- Added location breadcrumb `Connect > Servers` in terminal header.
- Updated split preset trigger to two-pane icon and preset list to text-only entries: `30/70`, `0/100`, `50/50`, `70/30`, `reset`.
- Implemented `0/100` to hide terminal shell pane when side panel is active.
- Added terminal shell horizontal scrolling and active-tab bottom alignment behavior.

### Change Log

- 2026-03-02: DS implementation for terminal UX optimization + Connect settings alignment with Epic 13.

### Proposed Follow-up Enhancements (Post-Review)

1. Session status fidelity
  - Upgrade Connections status dot to actual transport health state (`connecting / connected / disconnected`) instead of visual-only marker.

2. Selector efficiency
  - Add `Recent Sessions` subsection in `/connect` selector to reduce repeated server search.

3. Persistence hardening
  - Persist side-panel mode and split ratio per server.
  - Add bounded cache policy for preserved per-server side panels.

4. Docker tab consistency
  - Apply no-forced-scroll behavior uniformly across Images / Volumes / Networks / Compose when data count is small.

5. Keyboard accessibility
  - Add shortcuts for switching active connection, opening `+ New`, and closing current connection.
