...
describe('ServerCronPanel', () => {
	...
	it('supports editing an existing cron entry', async () => {
		...
		fireEvent.pointerDown(screen.getByRole('button', { name: 'Crontab actions for nightly-backup' }))
	...
	it('creates a cron entry from a minute cadence preset', async () => {
		...
		fireEvent.click(await screen.findAllByRole('button', { name: 'New Entry' }).then(buttons => buttons[0]))
