...
describe('reachability-policy', () => {
  it('uses interval multiplier with a minimum stale threshold', () => {
    ...
    expect(resolveReachabilityStaleAfterMs(10)).toBe(10 * REACHABILITY_STALE_MULTIPLIER * 60 * 1000)
  })
  ...
  it('marks records stale only after the threshold', () => {
    ...
    expect(isReachabilityStale('2026-04-11T10:06:00Z', staleAfterMs, nowMs)).toBe(false)
    expect(shouldBackgroundProbeReachability('2026-04-11T10:04:59Z', staleAfterMs, nowMs)).toBe(
      true
    )
  })
})
