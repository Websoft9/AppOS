...
describe('FeedsPage', () => {
	beforeEach(() => {
		...
		let feedRecords: Array<{
		...
		}> = [
			{
				...
				content_raw: '<p>Patch maintenance update with <strong>CVE</strong> fixes.</p><p>Includes service restart guidance.</p>',
					...
					bookmarkRecords = bookmarkRecords.map(item =>
						item.id === id
							? {
								...
								favicon_url: (options?.body?.favicon_url as string | undefined) || item.favicon_url,
							}
							: item
					)
			...
			if (path.startsWith('/api/feeds/items/') && path.endsWith('/state')) {
				...
				feedRecords = feedRecords.map(item =>
					item.id === itemID
						? {
							...item,
							read_state: (options?.body?.read_state as 'unread' | 'read' | undefined) ?? item.read_state,
	...
	it('loads feed sources, shows items, and persists reader actions', async () => {
		...
		expect(screen.getByText('Unified hub for RSS feeds, web content and bookmarks.')).toBeInTheDocument()
		...
		const articleRow = screen.getByText('Security Release 1').closest('div.rounded-lg.border.bg-card.px-4.py-3')
		expect(articleRow).not.toBeNull()
		expect(within(articleRow as HTMLElement).getByAltText('Vendor Releases favicon')).toBeInTheDocument()
		...
		expect(
			screen.getByText((_, element) => element?.textContent === 'Patch maintenance update with CVE fixes.')
		).toBeInTheDocument()
		...
		expect(within(shareDialog).getByRole('textbox', { name: 'Share URL' })).toHaveValue('https://example.com/releases/1?from=appos')
		await waitFor(() => {
			expect(within(shareDialog).getByRole('img', { name: 'QR code for Security Release 1' })).toHaveAttribute('src', 'data:image/png;base64,qr-code')
		})
		fireEvent.click(within(shareDialog).getByRole('button', { name: 'Copy URL' }))
		await waitFor(() => {
			expect(navigator.clipboard.writeText).toHaveBeenCalledWith('https://example.com/releases/1?from=appos')
		})
		...
		expect(screen.getByText('Centralize AppOS-related resources and personal favorite links here.')).toBeInTheDocument()
		...
		const copyButton = within(bookmarksList).getByRole('button', { name: 'Copy bookmark URL Saved Link' })
		...
		expect(within(bookmarksList).getByRole('button', { name: 'Edit bookmark Saved Link' })).toBeInTheDocument()
		...
		expect(within(bookmarksList).getByRole('link', { name: 'Saved Link Updated' })).toBeInTheDocument()
	...
	it('converts an article into a bookmark from article actions', async () => {
		...
		expect(screen.getByRole('link', { name: 'Security Release 1' })).toHaveAttribute('href', 'https://example.com/releases/1')
	})
	...
	it('shows bookmark title hover description, domain-only URL, and late-loading favicon from bookmark metadata', async () => {
					...
					items: [
						{
							...
							favicon_url: 'https://www.websoft9.com/favicon-32x32.png?v=0c3567d01883fb100f92704d2a7c1f24',
		...
		expect(websiteLink).toHaveAttribute('title', 'Websoft9（微聚云） 是一个创新的企业云应用托管平台。')
	...
	it('validates bookmark URLs before analyze and save', async () => {
		...
		fireEvent.click(screen.getByRole('button', { name: 'Save Bookmark' }))
		expect(screen.getByText('Enter a URL starting with http:// or https://.')).toBeInTheDocument()
		expect(sendMock).not.toHaveBeenCalledWith('/api/feeds/bookmarks', expect.objectContaining({ method: 'POST' }))
	})
	...
	it('falls back to legacy copy when navigator.clipboard is unavailable', async () => {
		...
		fireEvent.click(within(bookmarksList).getByRole('button', { name: 'Copy bookmark URL Saved Link' }))
