  ...
  useTranslation: () => ({
    t: (key: string, options?: Record<string, unknown>) => {
      const labels: Record<string, string> = {
        ...
        'connectors.columns.port': 'Port',
        'connectors.columns.reachability': 'Reachability',
        'connectors.columns.lastChecked': 'Last Checked',
        ...
        'connectors.authValues.none': 'No authentication',
        'connectors.authValues.basic': 'Username + Password',
        'connectors.authValues.bearer': 'API Key / Token',
        'connectors.authValues.api_key': 'API Key / Token',
        'connectors.authValues.token': 'API Key / Token',
...
describe('ConnectorsPage', () => {
  ...
  it('preloads relation options once and refreshes them when opening the create dialog', async () => {
    render(<ConnectorsPage />)
  ...
  it('auto-adds the default protocol and warns on mismatched schemes without blocking save', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Add External Service' }))
    const chooser = await screen.findByRole('dialog')
    clickChooserOption(chooser, 'REST API')
    ...
    fireEvent.change(endpointInput, { target: { value: 'http://api.example.com' } })

    expect(
      within(dialog).getByText(
        'The typed protocol http differs from the default https. You can still save this value.'
      )
    ).toBeInTheDocument()
  })
  ...
  it('keeps password secret inline for edit until the operator explicitly unlocks it', async () => {
    sendMock.mockImplementation((path: string) => {
      ...
      if (path === '/api/connectors/connector-1') {
        return Promise.resolve({ ok: true })
      }
      return Promise.resolve({ items: [] })
    })
  ...
  it('falls back to the editing item template_id when edit payload omits it', () => {
    expect(resolveConnectorTemplateId({}, { template_id: 'ses-smtp' })).toBe('ses-smtp')
    expect(
      resolveConnectorTemplateId({ template_id: 'generic-rest' }, { template_id: 'ses-smtp' })
    ).toBe('generic-rest')
  })
