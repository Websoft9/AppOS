...
sync_node_workspace() {
  ...
  if [ ! -d "${workspace_dir}/node_modules" ] || [ "${current_hash}" != "${previous_hash}" ]; then
    ...
    (cd "${workspace_dir}" && eval "${install_cmd}")
...
sync_node_workspace "${ROOT_DIR}/web" "${ROOT_DIR}/web/package-lock.json" "web-package-lock" "npm ci"
sync_node_workspace "${ROOT_DIR}/tests" "${ROOT_DIR}/tests/package-lock.json" "tests-package-lock" "npm ci"
