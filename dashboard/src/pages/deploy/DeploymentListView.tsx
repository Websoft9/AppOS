...
import type { ActiveFilterChip } from '@/pages/deploy/deploy-types'
...
type DeploymentListViewProps<TDeployment extends DeploymentListItem> = {
  search: string
  onSearchChange: (value: string) => void
  loading: boolean
  pagedItems: TDeployment[]
  page: number
  totalPages: number
  pageSize: number
  ...
  onToggleDeploymentSelection: (id: string, checked: boolean) => void
  onTogglePageSelection: (checked: boolean) => void
  allPageSelected: boolean
  somePageSelected: boolean
  onDeleteSelected: () => void
  onOpenDeployment: (id: string) => void
  renderActionMenu: (item: TDeployment) => React.ReactNode
}
...
export function DeploymentListView<TDeployment extends DeploymentListItem>({
  search,
  onSearchChange,
  loading,
  pagedItems,
  ...
  onOpenDeployment,
  renderActionMenu,
}: DeploymentListViewProps<TDeployment>) {
  return (
            ...
            <TableRow>
              <TableHead className="w-10">
                <Checkbox
                  ...
                  aria-label="Select visible deployments"
              ...
              <TableRow key={item.id} data-state={selectedIds.has(item.id) ? 'selected' : undefined}>
                <TableCell>
                  <Checkbox
                    ...
                    onCheckedChange={checked => onToggleDeploymentSelection(item.id, Boolean(checked))}
                  />
                </TableCell>
                <TableCell>
                  <div>
