...
import type { ServerEntry } from '@/pages/deploy/operations/operation-types'
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>Provide the Git repository, ref, and compose file path. The backend resolves the raw compose file and creates an operation through the shared flow.</DialogDescription>
        </DialogHeader>
        <div className="grid gap-4 py-2">
          ...
          <div className="rounded-lg border bg-muted/20 p-4">
            ...
            <div className="mt-1 text-xs text-muted-foreground">Optional. The header is used only to fetch the compose file and is not stored in operation records.</div>
        ...
        <DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)} disabled={submitting}>Cancel</Button><Button onClick={onSubmit} disabled={submitting || !repositoryUrl.trim() || !composePath.trim()}>{submitting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <GitBranch className="mr-2 h-4 w-4" />}Create Operation</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  )
}