...
type OperationDetailDialogProps = {
  open: boolean
  ...
  getUserLabel: (item: OperationRecord) => string
  getServerLabel: (item: OperationRecord) => string
  getServerHost: (item: OperationRecord) => string
  formatTime: (value?: string) => string
  statusVariant: (status: string) => 'default' | 'secondary' | 'destructive' | 'outline'
}

type OperationDetailContentProps = Omit<OperationDetailDialogProps, 'open' | 'onOpenChange'>
    ...
    <>
      {loading ? (
        ...
        <div className="space-y-3">
          <Card>
            ...
            <CardContent className="space-y-3">
              <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-4">
                <OverviewMetric
                  label="Status"
                  value={<Badge variant={statusVariant(operation.status)}>{operation.status}</Badge>}
                  valueClassName="text-xs"
                />
                <OverviewMetric label="Stream" value={streamStatus} />
                <OverviewMetric label="Started" value={formatTime(operation.started_at)} />
                <OverviewMetric label="Finished" value={formatTime(operation.finished_at)} />
              </div>
          ...
          <Card>
            <CardHeader className="pb-3"><CardTitle className="text-sm">Execution Timeline</CardTitle></CardHeader>
            <CardContent className="space-y-3">
              {(operation.lifecycle || []).length === 0 ? (
                <div className="text-xs text-muted-foreground">No lifecycle timeline available yet.</div>
              ) : (
                <>
                  <div className="overflow-x-auto pb-1">
                    <div className="flex min-w-max items-center gap-2">
                      {(operation.lifecycle || []).map((step, index, list) => (
                  ...
                  <div className="grid gap-2 lg:grid-cols-2 xl:grid-cols-3">
                    {(operation.lifecycle || []).filter(step => step.status !== 'pending').map(step => (
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-6xl">
        <DialogHeader>
          <DialogTitle>{contentProps.operation?.compose_project_name || 'Execution Detail'}</DialogTitle>
            <DialogDescription>Metadata, pipeline state, step timeline, and logs are stacked vertically to keep the reading flow stable.</DialogDescription>
        </DialogHeader>
        <OperationDetailContent {...contentProps} />
