...
export function AppDetailActionHistoryTable({ actions, buildActionDetailHref }: AppDetailActionHistoryTableProps) {
  return (
        ...
        {actions.map(action => (
          <TableRow key={action.id}>
            <TableCell className="min-w-[180px] align-top">
              ...
              <div className="line-clamp-1 text-xs text-muted-foreground">{action.error_summary || action.id}</div>
            </TableCell>
            ...
            <TableCell className="text-xs text-muted-foreground">{formatTime(action.started_at || action.pipeline?.started_at || action.created)}</TableCell>
            ...
            <TableCell className="max-w-[180px] truncate">{action.compose_project_name || '-'}</TableCell>
