    ...
    <TabsContent value="overview" className="space-y-2.5">
      <Card>
        <CardHeader className="pb-2.5">
          ...
          <CardDescription>Dense app signals for lifecycle, publication, runtime, and execution.</CardDescription>
        </CardHeader>
        <CardContent className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">
          ...
          <div className="rounded-2xl bg-muted/20 p-3"><div className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Execution</div><div className="mt-1 text-sm font-semibold">{app.current_pipeline?.family || app.last_operation || '-'}</div></div>
          <div className="rounded-2xl bg-muted/20 p-3"><div className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Health</div><div className="mt-1 text-sm font-semibold">{app.health_summary || app.runtime_status || '-'}</div></div>
        </CardContent>
      </Card>
      ...
      <div className="grid gap-2.5 xl:grid-cols-2">
        <Card>
          <CardHeader className="pb-2.5"><CardTitle>Access Snapshot</CardTitle></CardHeader>
          <CardContent className="space-y-1 text-sm text-muted-foreground">
            ...
            {canOpenServerDetail ? <Button variant="link" size="sm" className="h-auto w-fit px-0" onClick={openServerDetail}>Open server detail</Button> : null}
            <div>Certificate summary: not connected yet</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2.5"><CardTitle>Current Execution Summary</CardTitle></CardHeader>
          <CardContent className="space-y-1 text-sm text-muted-foreground">
            <div>{app.current_pipeline?.family || '-'} {app.current_pipeline?.current_phase ? `· ${app.current_pipeline.current_phase}` : ''}</div>
      ...
      <div className="grid gap-2.5 xl:grid-cols-2">
        <Card>
          <CardHeader className="pb-2.5"><CardTitle>Health Snapshot</CardTitle></CardHeader>
          <CardContent className="space-y-1 text-sm text-muted-foreground">
        ...
        <Card>
          <CardHeader className="pb-2.5"><CardTitle>Recent Actions</CardTitle></CardHeader>
          <CardContent className="space-y-1 text-sm text-muted-foreground">
            <div>Last operation: {app.last_operation || '-'}</div>
            <div>Current pipeline: {app.current_pipeline?.family || '-'}</div>
            <div className="flex flex-wrap gap-2 pt-1">
              {app.last_operation ? <Button variant="outline" size="sm" onClick={openOperationStatus}>Open Latest Execution</Button> : null}
              <Button variant="outline" size="sm" onClick={() => setTab('actions')}>Open Actions Tab</Button>
            </div>
          </CardContent>
        </Card>
      </div>
    ...
    <TabsContent value="access" className="space-y-2.5">
      <Card>
        ...
        <CardContent className="grid gap-2 text-sm md:grid-cols-2">
          ...
          <div><span className="text-muted-foreground">Server host:</span> {effectiveServerHost || '-'}</div>
          <div><span className="text-muted-foreground">Certificate status:</span> {primaryExposure?.certificate_id ? 'bound' : 'not bound'}</div>
          <div className="md:col-span-2 flex flex-col gap-1.5">
            <span className="text-muted-foreground">Domain access:</span>
            {primaryDomainUrl ? (
              <a href={primaryDomainUrl} target="_blank" rel="noreferrer" className="w-fit text-primary underline-offset-4 hover:underline">{primaryDomainUrl}</a>
            ) : <span>-</span>}
          </div>
          <div className="md:col-span-2 flex flex-col gap-1.5">
            <span className="text-muted-foreground">Public IP access:</span>
            {publicAccessUrl ? (
              <a href={publicAccessUrl} target="_blank" rel="noreferrer" className="w-fit text-primary underline-offset-4 hover:underline">{publicAccessUrl}</a>
            ) : <span>-</span>}
          </div>
          <div><span className="text-muted-foreground">Target port:</span> {primaryExposure?.target_port || '-'}</div>
          <div><span className="text-muted-foreground">Health state:</span> {primaryExposure?.health_state || '-'}</div>
          <div><span className="text-muted-foreground">Last verified:</span> {formatTime(primaryExposure?.last_verified_at)}</div>
          <div><span className="text-muted-foreground">Exposure state:</span> {primaryExposure?.publication_state || '-'}</div>
          {primaryExposure?.certificate_id ? (
            <div className="md:col-span-2 flex flex-wrap gap-2">
              <Button variant="outline" size="sm" asChild>
                <Link to="/certificates">Open Certificates</Link>
              </Button>
              <span className="text-xs text-muted-foreground">Certificate ID: {primaryExposure.certificate_id}</span>
            </div>
          ) : null}
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader className="border-b pb-3">
          ...
          <CardDescription>Operator-maintained login hints scoped to this app. This is not secret storage.</CardDescription>
          <CardAction>
            {!editingAccess ? (
            ...
            accessHintsPresent ? (
              <div className="space-y-3">
                ...
                <div className="space-y-1 text-sm">
                  ...
                  <div className="whitespace-pre-wrap text-sm">{displayValue(app.access_retrieval_method)}</div>
                </div>
                <div className="space-y-1 text-sm">
                  <div className="text-muted-foreground">Operator notes</div>
                  <div className="whitespace-pre-wrap text-sm">{displayValue(app.access_notes)}</div>
                </div>
              </div>
            ) : (
              <div className="rounded-2xl border border-dashed p-4 text-sm text-muted-foreground">
                No account hints saved yet. Add the default username, retrieval steps, and operator notes for this app.
              </div>
            )
          ) : (
            <>
              <div className="grid gap-3 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Input value={accessUsernameDraft} onChange={event => setAccessUsernameDraft(event.target.value)} placeholder="e.g. admin" />
                </div>
                <div className="space-y-2">
                  <div className="text-sm text-muted-foreground">Credential hint</div>
                  <Input value={accessSecretHintDraft} onChange={event => setAccessSecretHintDraft(event.target.value)} placeholder="e.g. initial password from welcome screen" />
                </div>
              </div>
              <div className="space-y-2">
                <div className="text-sm text-muted-foreground">Retrieval method</div>
                <Textarea className="min-h-[96px]" value={accessRetrievalMethodDraft} onChange={event => setAccessRetrievalMethodDraft(event.target.value)} placeholder="Describe how operators retrieve or reset the account credential." />
              </div>
              <div className="space-y-2">
                <div className="text-sm text-muted-foreground">Operator notes</div>
                <Textarea className="min-h-[96px]" value={accessNotesDraft} onChange={event => setAccessNotesDraft(event.target.value)} placeholder="Add app-scoped account notes, first login warnings, or rotation guidance." />
              </div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Button variant="outline" onClick={cancelAccessEditing} disabled={accessSaving}>Cancel</Button>
              </div>
            </>
          )}
        </CardContent>
...
export function AppDetailActionsTab({
  ...
  return (
    <TabsContent value="actions" className="space-y-2.5">
      <Card>
        <CardHeader className="pb-2.5"><CardTitle>Current and Recent</CardTitle></CardHeader>
        <CardContent className="grid gap-2 text-sm md:grid-cols-2">
          <div><span className="text-muted-foreground">Last operation:</span> {app.last_operation || '-'}</div>
          <div><span className="text-muted-foreground">Current pipeline:</span> {app.current_pipeline?.family || '-'}</div>
          <div><span className="text-muted-foreground">Current phase:</span> {app.current_pipeline?.current_phase || '-'}</div>
      ...
      <Card>
        <CardHeader className="border-b pb-3">
          ...
          <CardDescription>App-scoped action records pulled from the shared Actions subsystem.</CardDescription>
          <CardAction>
            <div className="flex flex-wrap gap-2">
              <Button variant="outline" size="sm" onClick={fetchActionHistory} disabled={actionsLoading}>
                {actionsLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}
                Refresh
              </Button>
              <Button variant="outline" size="sm" onClick={openAllActionsForApp}>Open in Actions</Button>
            </div>
          </CardAction>
        </CardHeader>
        <CardContent className="space-y-2.5 pt-4">
          <div className="grid gap-2 lg:grid-cols-[minmax(0,1fr)_170px_170px]">
            <div className="relative">
              ...
              <Input value={actionSearch} onChange={event => setActionSearch(event.target.value)} placeholder="Search this app's actions" className="pl-9" />
            </div>
            <Select value={actionStatusFilter} onValueChange={setActionStatusFilter}>
              <SelectTrigger><SelectValue placeholder="All statuses" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="all">All statuses</SelectItem>
                {actionStatusOptions.map(option => <SelectItem key={option} value={option}>{option}</SelectItem>)}
              </SelectContent>
            </Select>
            <Select value={actionTypeFilter} onValueChange={setActionTypeFilter}>
              <SelectTrigger><SelectValue placeholder="All action types" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="all">All action types</SelectItem>
                {actionTypeOptions.map(option => <SelectItem key={option} value={option}>{formatActionType(option)}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>

          {actionsLoading && scopedActions.length === 0 ? (
            <div className="rounded-2xl border p-4 text-sm text-muted-foreground">Loading action history...</div>
          ) : filteredScopedActions.length > 0 ? (
            <AppDetailActionHistoryTable actions={filteredScopedActions} buildActionDetailHref={buildActionDetailHref} />
            ...
            <div className="space-y-2 rounded-2xl border border-dashed p-4 text-sm text-muted-foreground">
              ...
              <div className="flex flex-wrap gap-2">
                {app.last_operation ? <Button variant="outline" onClick={openOperationStatus}>Open Latest Action Detail</Button> : null}
                <Button variant="outline" onClick={openAllActionsForApp}>Open Actions Page</Button>
              </div>
            </div>
          )}
        </CardContent>
