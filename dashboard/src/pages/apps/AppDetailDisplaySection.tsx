    ...
    <Card>
      <CardHeader className="border-b pb-4">
        <div>
          ...
          <CardDescription>Lightweight app metadata for icon, label, and tags. Stored locally in this browser for now.</CardDescription>
        </div>
        <CardAction>
      ...
      <CardContent className="space-y-4 pt-4">
        <div className="flex flex-wrap items-center gap-3 rounded-xl border bg-muted/20 p-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-1 flex flex-wrap gap-1.5">
              {tags.length > 0 ? tags.map(tag => <Badge key={tag} variant="secondary">{tag}</Badge>) : <span className="text-xs text-muted-foreground">No tags</span>}
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <div className="space-y-2">
            ...
            <Input value={iconValue} onChange={event => onIconChange(event.target.value)} placeholder="Emoji or image URL" />
          </div>
          <div className="space-y-2">
            <div className="text-sm text-muted-foreground">Label</div>
            <Input value={labelValue} onChange={event => onLabelChange(event.target.value)} placeholder="Short display label" />
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <Input value={tagsValue} onChange={event => onTagsChange(event.target.value)} placeholder="Comma-separated tags" />
        </div>

        <Alert>
          <AlertDescription>Until backend metadata fields exist, this display info is saved in the current browser only.</AlertDescription>
        </Alert>
      </CardContent>
    </Card>
  )
