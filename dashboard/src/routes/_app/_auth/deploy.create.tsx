...
const LazyCreateDeploymentPage = lazy(() =>
  import('@/pages/deploy/CreateDeploymentPage').then(module => ({ default: module.CreateDeploymentPage }))
)

function DeployCreateRoutePage() {
  const search = Route.useSearch()
  const entryMode = search.entry === 'compose' || search.entry === 'git-compose' || search.entry === 'docker-command' || search.entry === 'install-script'
    ? search.entry
    : undefined
  return (
    <Suspense fallback={<div className="p-6 text-sm text-muted-foreground">Loading Create Deployment...</div>}>
...
export const Route = createFileRoute('/_app/_auth/deploy/create')({
  ...
  validateSearch: (search: Record<string, unknown>) => ({
    entry: search.entry === 'compose' || search.entry === 'git-compose' || search.entry === 'docker-command' || search.entry === 'install-script'
      ? search.entry
      : undefined,
