...
- demonstrate how the official MySQL Docker image can be represented after Story 32.2 adaptation
...
- MySQL is a single-service application (the server is also the datastore), so there is no separate database service; `hasBuiltinDatabase` is `true`
- MySQL exposes a TCP wire protocol (3306), not a web UI; `web` is `false`

Validation baseline:

- `templates/tests/mysql.expected.json`
- `templates/tests/mysql-upstream-review.md`
