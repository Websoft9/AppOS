...
- demonstrate how the official MariaDB Docker image can be represented after Story 32.2 adaptation
...
- MariaDB is a single-service application (the server is also the datastore), so there is no separate database service; `hasBuiltinDatabase` is `true`
- MariaDB exposes a TCP wire protocol (3306), not a web UI; `web` is `false`

Validation baseline:

- `templates/tests/mariadb.expected.json`
- `templates/tests/mariadb-upstream-review.md`
