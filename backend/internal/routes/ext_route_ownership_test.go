...
// custom /api/ext/* route registrations must live under backend/internal/routes.
func TestExtRoutesOwnedByRoutesPackage(t *testing.T) {
	...
	if len(violations) > 0 {
		...
		t.Fatalf(
			"/api/ext route ownership violation: registrations must be defined in backend/internal/routes only.\n\n%s",
