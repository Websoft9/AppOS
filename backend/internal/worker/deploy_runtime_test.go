...
type mockDeploymentSFTPClient struct {
	mkdirCalls []string
	writeCalls []struct {
		path    string
		content string
...
func (m *mockDeploymentSFTPClient) MkdirAll(path string) error {
	m.mkdirCalls = append(m.mkdirCalls, path)
	return nil
}

func (m *mockDeploymentSFTPClient) WriteFile(path string, content string) error {
...
func (m *mockDeploymentSFTPClient) Close() error {
	return nil
}
...
func TestLocalDeploymentExecutorPrepareWorkspace(t *testing.T) {
	tmpDir := t.TempDir()
	projectDir := filepath.Join(tmpDir, "project")
	compose := "services:\n  web:\n    image: nginx:alpine\n"

	exec := localDeploymentExecutor{}
...
func TestDeploymentTunnelSSHPort(t *testing.T) {
	tests := []struct {
		name    string
		raw     string
		want    int
	...
	}{
		{
			...
			raw:     `[{"service_name":"ssh","tunnel_port":2222}]`,
			want:    2222,
			wantErr: false,
		},
		{
			name:    "missing ssh service",
			raw:     `[{"service_name":"http","tunnel_port":8080}]`,
			want:    0,
			wantErr: true,
		},
		{
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got, err := deploymentTunnelSSHPort(tc.raw)
			...
			if got != tc.want {
				t.Fatalf("deploymentTunnelSSHPort() = %d, want %d", got, tc.want)
			}
		})
	}
}

func TestSSHDeploymentExecutorPrepareWorkspaceUsesInjectedDependencies(t *testing.T) {
	mockClient := &mockDeploymentSFTPClient{}
	resolverCalled := false
	factoryCalled := false

	exec := sshDeploymentExecutor{
		app:      nil,
		serverID: "srv-1",
		...
		sftpFactory: func(ctx context.Context, cfg servers.ConnectorConfig) (deploymentSFTPClient, error) {
			factoryCalled = true
...
func TestSSHDeploymentExecutorPrepareWorkspaceResolverError(t *testing.T) {
