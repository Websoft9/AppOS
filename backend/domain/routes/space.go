...
// missing or unavailable.  sysconfig.GetGroup always returns a non-nil map, so
// callers using  quota, _ := sysconfig.GetGroup(...)  are always safe.
//
...
func handleSpaceQuota(e *core.RequestEvent) error {
	quota, _ := sysconfig.GetGroup(e.App, "space", "quota", defaultSpaceQuota)
	...
	return e.JSON(http.StatusOK, map[string]any{
		"max_size_mb":             sysconfig.Int(quota, "maxSizeMB", 10),
		"editable_formats":        strings.Split(spaceEditableFormats, ","),
		"upload_allow_exts":       sysconfig.StringSlice(quota, "uploadAllowExts"),
		"upload_deny_exts":        sysconfig.StringSlice(quota, "uploadDenyExts"),
		"max_upload_files":        maxUploadFiles,
		"max_per_user":            sysconfig.Int(quota, "maxPerUser", 100),
		"share_max_minutes":       sysconfig.Int(quota, "shareMaxMinutes", 60),
		"share_default_minutes":   sysconfig.Int(quota, "shareDefaultMinutes", 30),
		"reserved_folder_names":   strings.Split(spaceReservedFolderNames, ","),
		"disallowed_folder_names": sysconfig.StringSlice(quota, "disallowedFolderNames"),
	})
}
...
func handleSpaceFetch(e *core.RequestEvent) error {
	...
	// Read quota sysconfig.
	quota, _ := sysconfig.GetGroup(e.App, "space", "quota", defaultSpaceQuota)
	maxSizeMB := sysconfig.Int(quota, "maxSizeMB", 10)
	maxBytes := int64(maxSizeMB) * 1024 * 1024
	maxPerUser := sysconfig.Int(quota, "maxPerUser", 100)
	allowExts := sysconfig.StringSlice(quota, "uploadAllowExts")
	denyExts := sysconfig.StringSlice(quota, "uploadDenyExts")
