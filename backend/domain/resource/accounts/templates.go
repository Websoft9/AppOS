...
func loadKindBaseTemplate(kind string) (Template, error) {
	...
	if err != nil {
		return Template{}, fmt.Errorf("read provider account base template %s: %w", filePath, err)
	}
	base, err = applyTemplateOverlay(base, file)
	if err != nil {
		return Template{}, fmt.Errorf("merge provider account base template %s: %w", filePath, err)
	}
