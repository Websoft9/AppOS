...
func TestPlatformObserverCollectWritesPlatformTargets(t *testing.T) {
	...
	observer := monitor.NewPlatformObserver(app, func() monitor.RuntimeSnapshot {
	...
	for _, targetID := range []string{monitor.PlatformTargetAppOSCore, monitor.PlatformTargetWorker, monitor.PlatformTargetScheduler} {
...
func TestPlatformObserverCollectMarksStaleSchedulerDegraded(t *testing.T) {
	...
	observer := monitor.NewPlatformObserver(app, func() monitor.RuntimeSnapshot {
		return monitor.RuntimeSnapshot{
			...
			WorkerRunning:     true,
			SchedulerRunning:  true,
			SchedulerLastTick: now.Add(-monitor.PlatformSchedulerStaleThreshold).Add(-time.Second),
			LastDispatchAt:    now.Add(-monitor.PlatformSchedulerStaleThreshold).Add(-time.Second),
		}
	})
	observer.SetNowFunc(func() time.Time { return now })
	...
	record, err := app.FindFirstRecordByFilter(
		collections.MonitorLatestStatus,
		"target_type = {:targetType} && target_id = {:targetID}",
		map[string]any{"targetType": monitor.TargetTypePlatform, "targetID": monitor.PlatformTargetScheduler},
	)
	if err != nil {
		t.Fatal(err)
	}
