...
func TestBuildAppRuntimeEventUsesRegistryPolicy(t *testing.T) {
	...
	event := buildAppRuntimeEvent(entry, "Demo App", "server-1", "appinstance0001", "stopped", observedAt, summary)
...
func TestBuildServerRuntimeEventUsesRegistryPolicy(t *testing.T) {
	...
	event := buildServerRuntimeEvent(entry, "Primary Server", "server-1", "stopped", observedAt, summary)
