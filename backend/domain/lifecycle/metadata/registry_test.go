...
func TestDefinitionForSelectorChoosesSourceBuildInstall(t *testing.T) {
	...
	if definition.Nodes[1].Key != "hydrate_source_workspace" || definition.Nodes[2].Key != "prepare_source_runtime_workspace" {
		t.Fatalf("expected hydrate/prepare workspace sequence, got %s -> %s", definition.Nodes[1].Key, definition.Nodes[2].Key)
	}
	if definition.Nodes[3].Key != "build_artifact" || definition.Nodes[4].Key != "publish_artifact" {
		t.Fatalf("expected build/publish artifact sequence, got %s -> %s", definition.Nodes[3].Key, definition.Nodes[4].Key)
	}
	if definition.Nodes[5].Key != "create_candidate_release" {
		t.Fatalf("expected candidate release node, got %s", definition.Nodes[5].Key)
	}
