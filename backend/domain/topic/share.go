...
// It is returned by NewShareToken and must be applied to the Topic record
// by the caller (route handler or application service).
type Share struct {
	Token     string
	ExpiresAt time.Time
}

// NewShareToken generates a cryptographically random share token with the
// given duration in minutes, validated against the configured limits.
//
// Rules:
