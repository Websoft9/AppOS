...
// ImageBuild builds a local image from a source directory and tags it.
func (c *Client) ImageBuild(ctx context.Context, tag string, contextDir string) (string, error) {
	return c.exec.Run(ctx, "docker", "build", "-t", tag, contextDir)
}
...
// ImageTag tags a local image reference with a target reference.
func (c *Client) ImageTag(ctx context.Context, sourceRef string, targetRef string) (string, error) {
	return c.exec.Run(ctx, "docker", "image", "tag", sourceRef, targetRef)
}

// ImagePush pushes an image reference to its configured registry.
func (c *Client) ImagePush(ctx context.Context, ref string) (string, error) {
	return c.exec.Run(ctx, "docker", "image", "push", ref)
}
